...
class DatasetPathError(Exception):
    ...
    def __str__(self):
        return (
            "DatasetPathError: The images directory or label directory cannot be None!"
        )
...
def parse_parameters():  # pragma: no cover
    ...
    parser.add_argument(
        ...
        dest="data_dir",
        required=True,
        help="""--- Path to directory containing both images and labels ---""",
    )
    parser.add_argument(
        "--image_dir",
        action="store",
    ...
    parser.add_argument(
        ...
        dest="label_dir",
        required=True,
        help="""--- Path to directory containing labels/annotations ---""",
    )
    parser.add_argument(
        "--class_file",
        action="store",
        dest="class_file",
        required=True,
        help="""--- Path to .txt or .csv file containing class names ---""",
    )
    parser.add_argument(
        "--output_dir",
        action="store",
...
def validate_arguments(
    data_directory, img_directory, lbl_directory, cls_file, valid_size
):
    ...
    """
    if (
        data_directory.lower() == "none"
        and img_directory.lower() == "none"
        and lbl_directory.lower() == "none"
    ):
        raise NoneDatasetError

    if data_directory.lower() == "none":
        if (img_directory.lower() != "none" and lbl_directory.lower() == "none") or (
            img_directory.lower() == "none" and lbl_directory.lower() != "none"
        ):
...
def validate_dataset(
    data_dir_list, img_dir_list, lbl_dir_list, img_formats, class_list
):
    ...
    if len(data_dir_list):
        image_list = [
            file for file in data_dir_list if any(fmt in file for fmt in img_formats)
        ]
        label_list = [file for file in data_dir_list if ".txt" in file]
    else:
        image_list = [
            file for file in img_dir_list if any(fmt in file for fmt in img_formats)
        ]
...
def train_valid_split(images, labels, valid_size):
    ...
    train_images, val_images, train_labels, val_labels = train_test_split(
        images, labels, test_size=float(valid_size), random_state=42
    )
...
def create_dataset_config(class_list, output_dir, config_dict):  # pragma: no cover
...
def data_preparation_main():  # pragma: no cover
    """Command line execution."""
    # Get input parameters and perform validation
    args = parse_parameters()
    validate_arguments(
        args.data_dir, args.image_dir, args.label_dir, args.class_file, args.valid_size
    )
    image_formats = ["bmp", "jpg", "jpeg", "png", "tif", "tiff", "dng", "webp", "mpo"]

    # Read config file
    with open(
        os.path.dirname(os.path.abspath(__file__)) + "/data_preparation_config.yaml",
        "r",
    ) as file:
    ...
    images, labels = validate_dataset(
        data_dir_contents, img_dir_contents, lbl_dir_contents, image_formats, class_list
    )

    # Split dataset into training and validation sets
    train_images, valid_images, train_labels, valid_labels = train_valid_split(
        images, labels, args.valid_size
    )
