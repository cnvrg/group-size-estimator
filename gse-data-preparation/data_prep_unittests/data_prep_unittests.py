...
class TestDataPreparation(unittest.TestCase):
    def setUp(self):
        ...
        self.data_list = ["1.jpg", "1.txt", "2.jpg", "2.txt", "3.jpg", "3.txt", "4.jpg", "4.txt", "5.jpg", "5.txt"]
...
class TestValidateArguments(TestDataPreparation):
    ...
    def test_validation_size_error(self):
        ...
        with self.assertRaises(ValidationSizeError):
            validate_arguments("None", self.img_dir, self.lbl_dir, self.class_txt, "0.6")
...
class TestValidateDataset(TestDataPreparation):
    def test_data_dir_input(self):
        ...
        result = validate_dataset(self.data_list, [], [], self.img_formats, self.class_list)
        self.assertIsInstance(result[0], list)
        self.assertIsInstance(result[1], list)
        self.assertEqual([len(result[0]), len(result[1])], [self.num_images, self.num_labels])
    
    def test_img_lbl_input(self):
        """Checks for appropriate output when images and labels are present in different directories"""
        result = validate_dataset([], self.img_list, self.lbl_list, self.img_formats, self.class_list)
        self.assertIsInstance(result[0], list)
        self.assertIsInstance(result[1], list)
        self.assertEqual([len(result[0]), len(result[1])], [self.num_images, self.num_labels])
    ...
    def test_dataset_size_error(self):
        ...
        with self.assertRaises(DatasetSizeError):
            validate_dataset([], self.img_list_v1, self.lbl_list_v1, self.img_formats, self.class_list)
    ...
    def test_dataset_naming_error(self):
        ...
        with self.assertRaises(DatasetNamingError):
            validate_dataset([], self.img_list_v1, self.lbl_list_v2, self.img_formats, self.class_list)
    ...
    def test_number_of_classes_error(self):
        ...
        with self.assertRaises(NumberOfClassesError):
            validate_dataset([], self.img_list, self.lbl_list, self.img_formats, self.class_list_v1)
