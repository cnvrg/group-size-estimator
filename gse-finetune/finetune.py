...
def parse_parameters(): # pragma: no cover
    ...
    parser.add_argument("--model_weights", action="store", dest="model_weights", required=False, default="yolov5s.pt", help="""--- Pre-trained model weights ---""")
    parser.add_argument("--img_size", action="store", dest="img_size", required=False, default=640, help="""--- Size of images for training and validation. Images will be resized to this size ---""")
    parser.add_argument("--batch_size", action="store", dest="batch_size", required=False, default=16, help="""--- Batch size for training the model ---""")
    parser.add_argument("--num_epochs", action="store", dest="num_epochs", required=False, default=5, help="""--- Number of epochs to train the model for ---""")
...
def finetune_main(): # pragma: no cover
    ...
    with open(os.path.dirname(os.path.abspath(__file__)) + "/finetune_config.yaml", "r") as file:
        config_dict = yaml.load(file, Loader=yaml.FullLoader)
    validate_file_locations(config_dict["images_loc"], config_dict["labels_loc"], config_dict["config_loc"])
    ...
    command = f"python3 train.py --weights {args.model_weights} --img {args.img_size} --batch {args.batch_size} --data {dataset_yaml_loc} --project {project_loc}"
    os.system(command)
