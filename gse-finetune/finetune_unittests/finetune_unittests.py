...
class TestValidateFileLocations(TestFineTune):
    def test_dir_not_found_error(self):
        ...
        with self.assertRaises(DirectoryNotFoundError):
            validate_file_locations(self.false_img_path, self.config["labels_loc"], self.config["config_loc"])
        with self.assertRaises(DirectoryNotFoundError):
            validate_file_locations(self.config["images_loc"], self.false_lbl_path, self.config["config_loc"])
    ...
    def test_config_not_found_error(self):
        ...
        with self.assertRaises(ConfigNotFoundError):
            validate_file_locations(self.config["images_loc"], self.config["labels_loc"], self.config["config_loc"])
